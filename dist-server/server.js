...
async function getUserById(id) {
    let rows;
    try {
        [rows] = await pool.query('SELECT id, username, firstname, lastname, email, dob, avatar_url, bio, interests FROM users WHERE id = ?', [id]);
    }
    catch (_e) {
        // bio/interests columns may not exist yet — fall back to base columns
        [rows] = await pool.query('SELECT id, username, firstname, lastname, email, dob, avatar_url FROM users WHERE id = ?', [id]);
    }
...
app.put('/api/shelves/:id', async (req, res) => {
    ...
    try {
        const [rows] = await pool.query('SELECT id, user_id FROM user_shelves WHERE id = ? LIMIT 1', [shelfId]);
        if (!Array.isArray(rows) || rows.length === 0)
...
app.get('/api/friends/status', async (req, res) => {
    ...
    try {
        const [rows] = await pool.query(`SELECT id, requester_id, addressee_id, status
       FROM friendships
       WHERE (requester_id = ? AND addressee_id = ?) OR (requester_id = ? AND addressee_id = ?)
       LIMIT 1`, [userId, otherUserId, otherUserId, userId]);
        if (!Array.isArray(rows) || rows.length === 0)
...
app.post('/api/friends/request', async (req, res) => {
    ...
    try {
        const [existing] = await pool.query(`SELECT id, status FROM friendships
       WHERE (requester_id = ? AND addressee_id = ?) OR (requester_id = ? AND addressee_id = ?)
       LIMIT 1`, [fromUserId, toUserId, toUserId, fromUserId]);
        ...
        const [result] = await pool.query(`INSERT INTO friendships (requester_id, addressee_id, status) VALUES (?, ?, 'pending')`, [fromUserId, toUserId]);
...
app.get('/api/friends', async (req, res) => {
    ...
    if (!Number.isInteger(userId) || userId < 1)
        return res.status(400).json({ error: 'valid user_id is required' });
    try {
        const [rows] = await pool.query(`SELECT u.id, u.username, u.avatar_url
       FROM friendships f
       JOIN users u ON u.id = IF(f.requester_id = ?, f.addressee_id, f.requester_id)
       WHERE (f.requester_id = ? OR f.addressee_id = ?) AND f.status = 'accepted'`, [userId, userId, userId]);
        const friends = (Array.isArray(rows) ? rows : []).map(r => ({
            id: r.id,
            username: r.username,
...
app.patch('/api/friends/:id/respond', async (req, res) => {
    ...
    try {
        const [rows] = await pool.query('SELECT id, requester_id, addressee_id, status FROM friendships WHERE id = ? LIMIT 1', [requestId]);
        if (!Array.isArray(rows) || rows.length === 0)
...
app.delete('/api/friends', async (req, res) => {
    ...
    try {
       ...
       WHERE (requester_id = ? AND addressee_id = ?) OR (requester_id = ? AND addressee_id = ?)`, [userId, friendId, friendId, userId]);
        res.json({ success: true });
    }
    catch (e) {
...
app.post('/api/communities/:id/members/add', async (req, res) => {
    ...
    const userIds = Array.isArray(body.user_ids)
        ? body.user_ids.map(Number).filter(n => Number.isInteger(n) && n > 0)
        : [];
    if (!Number.isInteger(requestingUserId) || requestingUserId < 1)
        return res.status(400).json({ error: 'valid requesting_user_id is required' });
    if (userIds.length === 0)
    ...
    try {
        ...
        await pool.query('INSERT IGNORE INTO community_members (user_id, community_id, community_role) VALUES ?', [values]);
...
app.delete('/api/threads/:threadId/posts/:postId', async (req, res) => {
    ...
    if (!Number.isInteger(requestingUserId) || requestingUserId < 1)
        return res.status(400).json({ error: 'valid requesting_user_id is required' });
    try {
        const [postRows] = await pool.query('SELECT id, thread_id, user_id FROM forum_posts WHERE id = ? AND thread_id = ? LIMIT 1', [postId, threadId]);
        ...
        if (post.user_id !== requestingUserId) {
            ...
            const [threadRows] = await pool.query('SELECT community_id FROM forum_threads WHERE id = ? LIMIT 1', [threadId]);
            if (!Array.isArray(threadRows) || threadRows.length === 0)
                return res.status(404).json({ error: 'Thread not found' });
            const communityId = threadRows[0].community_id;
            const [communityRows] = await pool.query('SELECT owner_id FROM communities WHERE id = ? LIMIT 1', [communityId]);
            const [cmRows] = await pool.query('SELECT community_role FROM community_members WHERE user_id = ? AND community_id = ? LIMIT 1', [requestingUserId, communityId]);
            const isOwner = Array.isArray(communityRows) && communityRows.length > 0 &&
                communityRows[0].owner_id === requestingUserId;
            const isAdmin = Array.isArray(cmRows) && cmRows.length > 0 &&
                cmRows[0].community_role === 'admin';
            if (!isOwner && !isAdmin)
