...
app.get('/api/users/search', async (req, res) => {
  ...
  try {
    ...
    const users = (Array.isArray(rows) ? rows as any[] : []).map((r: any) => ({
      id: r.id,
      username: r.username,
...
app.get('/api/communities/:id/threads', async (req, res) => {
  ...
  try {
    const [rows] = await pool.query(
      `SELECT ft.id, ft.community_id, ft.title, ft.created_by, ft.created_at,
...
app.post('/api/communities/:id/threads', async (req, res) => {
  ...
  try {
    const [result] = await pool.query<mysql.ResultSetHeader>(
      'INSERT INTO forum_threads (community_id, title, created_by) VALUES (?, ?, ?)',
      [communityId, title, userId]
    );
    const [rows] = await pool.query(
      `SELECT ft.id, ft.community_id, ft.title, ft.created_by, ft.created_at,
              u.username, 0 AS post_count
       FROM forum_threads ft JOIN users u ON u.id = ft.created_by WHERE ft.id = ?`,
      [result.insertId]
    );
...
app.get('/api/threads/:id/posts', async (req, res) => {
  ...
  try {
    const [rows] = await pool.query(
      `SELECT fp.id, fp.thread_id, fp.user_id, fp.content, fp.created_at,
...
app.post('/api/threads/:id/posts', async (req, res) => {
  ...
  if (!Number.isInteger(threadId) || threadId < 1)
    return res.status(400).json({ error: 'Invalid thread id' });
  const content = typeof (req.body || {}).content === 'string' ? (req.body.content as string).trim() : '';
  const userId = Number((req.body || {}).user_id);
...
app.get('/api/friends/pending', async (req, res) => {
  ...
  try {
    const [rows] = await pool.query(
      `SELECT f.id AS request_id, u.id, u.username, u.avatar_url
       FROM friendships f
       JOIN users u ON u.id = f.requester_id
       WHERE f.addressee_id = ? AND f.status = 'pending'`,
      [userId]
    );
    const requests = (Array.isArray(rows) ? rows as any[] : []).map(r => ({
